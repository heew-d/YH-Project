...
def region_of_interest(frame):
    ...
    polygon = np.array([[
        ...
        (int(width*0.0),  int(height*0.60)),   # 왼쪽 위
        (int(width), int(height*0.60)),        # 오른쪽 위
...
def map_coordinates(frame, parameters): 
    ...
    y2 = int(height*0.20)  # 프레임 중앙에서 아래쪽으로 점 찍기
...
def Orchestrator(frame):
    ...
    if lane_lines!=None:
        ...
        cv2.putText(final_frame, str(weight_angle), (int(width*0.10), int(height*0.40)), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 255, 255), 1, cv2.LINE_AA)
