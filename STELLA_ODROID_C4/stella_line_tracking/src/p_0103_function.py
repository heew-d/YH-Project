...
def region_of_interest(frame):
    ...
    polygon = np.array([[
        ...
        (int(width*0.0),  int(height*0.30)),   # Top-left point # 왼쪽 위
        (int(width), int(height*0.30)),    # Top-right point # 오른쪽 위
...
def histogram(frame):
    ...
    midpoint = np.int32(histogram.shape[0]/2)     # Find mid point on histogram
...
def map_coordinates(frame, parameters):
    ...
    y2 = int(height*0.50)  # Make point from middle of the frame down  
...
def test4_Import_lib(frame):
    ...
    if lane_lines!=None:
        ...
        return final_frame, weight_angle  # Return final frame
    else:
        return frame, -1
        # return frame, -20
